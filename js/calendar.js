...
var JTC = (function() {
	var j = function() {},
		...
		util = {
			...
			getScrollPos: function() {
				...
				if (browser.IE || browser.OPERA) {
					...
					if(typeof window.scrollX=='undefined'){//ie11
						var el = this.getDocumentElement();
						x = el.scrollLeft;
						y = el.scrollTop;
					}else{
		...
		events = {
			...
			showSelector: function(flag) {
				...
				var _style = 'position:absolute; z-index:20010; background-color:#FFFFFF; width:78px;height:120px; top:' + coord.y + 'px; left:' + coord.x + 'px; border:1px solid #A77AEB; padding:2px 2px;';
		...
		dialog = function() {
			...
			this.getWeekDayPanel = function() {
				...
				var style = 'height:23px; border-top:0px; border-bottom:1px solid #008FEE; borderright:0px; border-left:0px;background-color:#FFFFFF;';
				for (var i = 0; i < 7; i++) {
	...
	j.setToday = function(dateObj) {
		config.today = util.setDate(dateObj);
	};
	j.setDateRange = function(minDate, maxDate, ranged) {
	...
	j.setDateFormat = function(format) {
		config.format = format || config.format;
	};
	j.setStartDay = function(date) {
